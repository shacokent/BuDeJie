...
- (void)awakeFromNib{
    ...
    [self.movieImageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(seeBigPucture)]];
}

- (void)setRootbaseItem:(SKTopicItem *)rootbaseItem{
    _rootbaseItem = rootbaseItem;
    [self.movieImageView SK_setOriginImageWithURL:rootbaseItem.image andTuhumbImageWithURL:rootbaseItem.image andPlaceholderImageName:@"post_placeholderImage"completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
